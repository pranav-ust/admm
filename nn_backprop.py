...
#dst=transform.resize(digits.data,(8,8))
#plt.matshow(digits.images[0])
#plt.show()
print(type(digits.images[0]))
print(digits.images[0].shape)
#dst=transform.resize(digits.images[0], (8,8))
'''
...
class BPNeuralNetwork:
    ...
    def train(self, cases, labels, limit=100, learn=0.05, correct=0.1):
        for j in range(limit):
            error = 0.0
            for i in range(len(cases)):
                label = labels[i]
                case = cases[i]
                #print("the global error is",self.back_propagate(case, label, learn, correct))
